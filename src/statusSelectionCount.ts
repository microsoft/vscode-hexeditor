...
export default class StatusSelectionCount extends Disposable {
	...
	update({ focused, selected }: ISelectionState): void {
		...
		if (nFocus && nSelected) {
			this.item.text = vscode.l10n.t("Byte 0d{0}/0x{1} (0d{2}/0x{3} selected)", nFocus, nFocus.toString(16).toUpperCase(), nSelected, nSelected.toString(16).toUpperCase());
		} else if (nSelected) {
			this.item.text = vscode.l10n.t("0d{0}/0x{1} selected", nSelected, nSelected.toString(16).toUpperCase());
		} else if (nFocus) {
			this.item.text = vscode.l10n.t("Byte 0d{0}/0x{1}", nFocus, nFocus.toString(16).toUpperCase());
		} else {
			this.item.hide();
			return;
		}
